...
DenyBucketDeletionPolicy=$(aws cloudformation describe-stacks --stack-name edx-project-iam-stack \
--query 'Stacks[0].Outputs[?OutputKey==`DenyBucketDeletionPolicy`].OutputValue' --output text)
AllowBucketDeletionPolicy=$(aws cloudformation describe-stacks --stack-name edx-project-iam-stack \
--query 'Stacks[0].Outputs[?OutputKey==`AllowBucketDeletionPolicy`].OutputValue' --output text)
